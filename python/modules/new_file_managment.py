...
Operations on paths.
"""  
def path_unification(in_path): # Normalize the path for your system, at the moment feature is available only for Windows
    windows = True
...
"""
def remove(path): # file or directory
...
def copy_dir_constantly(in_path, out_path, sleep_time):
    while True:
...
"""
def check_dir_completeness(in_path, required_files, sleep_time): # checking out if the given list of files exist in given directory (path)
    filelist = []
    while True:
...
def file_extension_verification(filename, extension): #checking out if the given file have got given extension
    if filename.endswith(extension):
        return True
    else:
        return False
def get_file_extension(filename): #extracting extension from file i.e. ".py" from "foo.py"
    extension = os.path.splitext(filename)[1]
    return extension
    
def extract_name(path): #extract file/dir name from path i.e. "foo.txt" from "path/to/foo.txt"
